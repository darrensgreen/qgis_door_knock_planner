...
class DoorKnockPlannerAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterFileDestination(
            self.OUTPUT_CSV, self.tr('Door Knock List (CSV)'), 'csv',
            optional=True
        ))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        extracted_layer = extract_result['OUTPUT']
        ...
        if rank_field:
            ...
            layer_to_cluster = sorted_result['OUTPUT']
        else:
        ...
        clustered_addresses = clustered_result['OUTPUT']

        feedback.pushInfo("Step 4: Preparing final output...")
        route_fields = QgsFields()
        for field in road_layer.fields():
        ...
        for i in range(num_crews):
            ...
            crew_addresses = crew_addresses_result['OUTPUT']
            ...
            snapped_layer = snapped_point_result['OUTPUT']
            ...
            temp_route_layer = route_result['OUTPUT']
            
            route_features = []
            for feature in temp_route_layer.getFeatures():
                new_feature = QgsFeature(route_fields)
                new_feature.setGeometry(feature.geometry())
                
                for field in feature.fields():
                    field_name = field.name()
                    idx = new_feature.fields().indexOf(field_name)
                    if idx != -1:
                        new_feature.setAttribute(idx, feature.attribute(field_name))
                new_feature.setAttribute('crew_id', i + 1)
                route_features.append(new_feature)

            if route_features:
                routes_sink.addFeatures(route_features)
            else:
                feedback.pushWarning(f"No routes could be calculated for Crew #{i+1}.")
        
        return {self.OUTPUT_ROUTES: routes_dest_id, self.OUTPUT_CSV: ''}
